        ...
        <div class="db-card-body">
            <form @submit.prevent="save">
                <div class="form-row">
                    ...
                    <div class="form-col-12 sm:form-col-6">
                        <label for="main_banner" class="db-field-title">
                            {{ $t("label.main_banner") }} (1920px,1080px)
                        </label>
                        <input
                            @change="changeMainBanner"
                            v-bind:class="errors.main_banner ? 'invalid' : ''"
                            id="main_banner"
                            type="file"
                            class="db-field-control"
                            ref="themeMainBannerProperty"
                            accept="image/png, image/jpeg, image/jpg"
                        />
                        <small class="db-field-alert" v-if="errors.main_banner">
                            {{ errors.main_banner[0] }}
                        </small>
                        <img
                            class="w-full h-auto object-fill rounded-lg mt-2"
                            alt="main banner"
                            v-if="theme_main_banner_reader"
                            :src="theme_main_banner_reader"
                        />
                    </div>
...
export default {
    ...
    methods: {
        ...
        changeLogo: function (e) {
            ...
            this.readFile(this.theme_logo, (result) => {
                this.theme_logo_reader = result;
            });
        },
        changeFavIcon: function (e) {
            this.theme_favicon_logo = e.target.files[0];
            this.readFile(this.theme_favicon_logo, (result) => {
                this.theme_favicon_logo_reader = result;
            });
        },
        changeFooterLogo: function (e) {
            this.theme_footer_logo = e.target.files[0];
            this.readFile(this.theme_footer_logo, (result) => {
                this.theme_footer_logo_reader = result;
            });
        },
        changeMainBanner: function (e) {
            this.theme_main_banner = e.target.files[0];
            this.readFile(this.theme_main_banner, (result) => {
                this.theme_main_banner_reader = result;
            });
        },
        list: function () {
            ...
            this.$store
                ...
                .then((res) => {
                    ...
                    this.theme_main_banner_reader = res.data.data.theme_main_banner;
        ...
        save: function () {
            try {
                ...
                if (this.theme_main_banner) {
                    fd.append("theme_main_banner", this.theme_main_banner);
                }
                this.loading.isActive = true;
                this.$store
                    .dispatch("theme/save", {
                    ...
                    .then((res) => {
                        ...
                        this.$refs.themeMainBannerProperty.value = null;
                    })
                    .catch((err) => {
                        this.loading.isActive = false;
