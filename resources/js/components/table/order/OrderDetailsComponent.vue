...
export default {
    ...
    mounted() {
        ...
        if (this.$route.params.id) {
            ...
            this.socketOrderStatus();
            this.socketOrderPaymentStatus();
        }
    },
    methods:{
        socketOrderStatus(){
            ...
            window.Echo.channel(`order-status-updated-${orderId}`)
                .listen('.OrderStatusUpdated', (event) => {
                    // console.log('OrderStatus Updated:', event);
                    this.order.status = event?.status;
                });
        },
        socketOrderPaymentStatus(){
            const orderId = this.$route.params.id ?? null;
            if (!orderId) return
            window.Echo.channel(`order-payment-updated-${orderId}`)
