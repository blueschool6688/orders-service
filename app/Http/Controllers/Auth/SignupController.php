...
class SignupController extends Controller
{
    ...
    public function clientSignup(ClientSignUpRequest $request) : \Illuminate\Http\Response | \Illuminate\Contracts\Foundation\Application | \Illuminate\Contracts\Routing\ResponseFactory
    {
        $isPhoneSignup = $request->filled('phone');
        $user = null;
        if ($isPhoneSignup) {
        ...
        if ($user) {
            $user->name = $name;
            $user->username = Str::slug($name).time();
            if (!$isPhoneSignup) {
                $user->email = $request->post('email');
            } else {
            ...
            if ($isPhoneSignup) {
                ...
                $userData['country_code'] = $request->post('country_code');
            } else {
