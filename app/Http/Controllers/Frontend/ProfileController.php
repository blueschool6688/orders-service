...
class ProfileController extends Controller
{
    ...
    public function clientUpdate(ClientProfileRequest $request) : UserResource | \Illuminate\Http\Response | \Illuminate\Contracts\Foundation\Application | \Illuminate\Contracts\Routing\ResponseFactory
    {
        try{
            return new UserResource($this->profileService->clientUpdate($request));
        }
        catch (\Exception $exception){
            return response(['status' => false, 'message' => $exception->getMessage()], 422);
        }
    ...
    public function clientChangePassword(ClientChangePasswordRequest $request) : UserResource | \Illuminate\Http\Response | \Illuminate\Contracts\Foundation\Application | \Illuminate\Contracts\Routing\ResponseFactory
    {
        try {
            return new UserResource($this->profileService->changeClientPassword($request));
        }
        catch (\Exception $exception){
            return response(['status' => false, 'message' => $exception->getMessage()], 422);
        }
