...
class ThemeService
{
    ...
    public function update(ThemeRequest $request)
    {
        ...
        try {
            ...
            if ($request->theme_main_banner){
                $setting = ThemeSetting::where('key', 'theme_main_banner')->first();
                $setting->clearMediaCollection('theme-banner');
                $setting->addMediaFromRequest('theme_main_banner')->toMediaCollection('theme-banner');
            }
            return $this->list();
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
